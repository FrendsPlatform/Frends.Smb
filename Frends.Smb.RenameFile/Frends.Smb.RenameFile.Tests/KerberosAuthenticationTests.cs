...
public class KerberosAuthenticationTests
{
    ...
    public void Setup()
    {
        var file1 = Path.Combine(testFilesPath, "file1.txt");
        var renamed = Path.Combine(testFilesPath, "renamed.txt");

        if (File.Exists(renamed))
            File.Delete(renamed);

        File.WriteAllText(file1, "File One");
    ...
    [Test]
    public void RenameFile_Kerberos_Success()
    {
        input = new Input
        {
            Path = @"file1.txt",
        ...
        Assert.That(File.Exists(Path.Combine(testFilesPath, "renamed.txt")), Is.True);
        Assert.That(File.Exists(Path.Combine(testFilesPath, "file1.txt")), Is.False);
    }

    [Test]
    public void RenameFile_KerberosTicketCache_Success()
    {
        input = new Input
        {
            Path = @"file1.txt",
    ...
    [Test]
    public void RenameFile_KerberosTicketCache_MissingCacheFile_Fails()
    {
        input = new Input
        {
            Path = @"file1.txt",
    ...
    [Test]
    public void RenameFile_Kerberos_WrongPassword_Fails()
    {
