...
public class KerberosAuthenticationTests
{
    ...
    [Test]
    public void CreateDirectory_Kerberos_Success()
    {
        input = new Input { DirectoryPath = "newDir" };

        var result = Smb.CreateDirectory(input, connection, options, CancellationToken.None);

        Assert.That(result.Success, Is.True, result.Error?.Message);
        Assert.That(Directory.Exists(Path.Combine(testFilesPath, "newDir")), Is.True);
    }

    [Test]
    public void CreateDirectory_KerberosTicketCache_Success()
    {
        input = new Input { DirectoryPath = "newDir" };
    ...
    [Test]
    public void CreateDirectory_KerberosTicketCache_MissingCacheFile_Fails()
    {
        input = new Input { DirectoryPath = "newDir" };
        connection.AuthenticationMode = AuthenticationMode.KerberosTicketCache;
    ...
    [Test]
    public void CreateDirectory_Kerberos_WrongPassword_Fails()
    {
